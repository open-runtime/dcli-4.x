...
void main() {
  ...
  test('ask.any - throws', () async {
    ...
    await check(validator.validate('abc')).throws<AskValidatorException>((it) => it.has((e) => e.message, red('Invalid FQDN.')));
  });
  ...
  test('ask.all - failure', () async {
    ...
    await check(validator.validate('9')).throws<AskValidatorException>((it) => it.has((e) => e.message, red('The number must be greater than or equal to 10.')));
  });

  test('ask.integer - failure', () async {
    const validator = Ask.integer;

    await check(validator.validate('a')).throws<AskValidatorException>((it) => it.has((e) => e.message, red('Invalid integer.')));
  ...
  test('ask.url - default protocols', () async {
    ...
    await check(validator.validate('http://onepub.dev')).throws<AskValidatorException>((it) => it.has((e) => e.message, red('Invalid URL.')));
  });
  ...
  test('ask.url - custom protocols', () async {
    ...
    await check(validator.validate('def://onepub.dev')).throws<AskValidatorException>((it) => it.has((e) => e.message, red('Invalid URL.')));
  });
}
